...
int BestFirstSearch:: calculate_novelty(TreeNode * child_node){

    return MAXI - check_novelty_1( child_node->state.getRAM()); 
}

int BestFirstSearch:: explore_child(TreeNode* curr_node){
...
void BestFirstSearch::expand_tree(TreeNode* start_node) {
	...
	num_simulated_steps +=  explore_child( curr_node );
