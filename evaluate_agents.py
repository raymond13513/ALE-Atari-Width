...
def main() :
    ...
    command_template = """./ale -display_screen false -discount_factor 0.995 -randomize_successor_novelty true -record_trajectory %(record)s -max_sim_steps_per_frame %(step)d -max_num_frames 6000 -restricted_action_set false -random_seed %(i)d %(agent_cmd)s %(rom_path)s"""

    inputs = []
    for step in [x * inc for x in range(1,ran+1)]:
